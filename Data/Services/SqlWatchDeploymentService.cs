...
namespace SqlHealthAssessment.Data.Services
{
    public class SqlWatchDeploymentService
    {
        ...
        public async Task<DeploymentResult> TestConnectionAsync(IProgress<DeploymentProgress> progress = null, CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                progress?.Report(new DeploymentProgress { Status = "Testing connection...", Percentage = 0 });

                using var connection = new SqlConnection(_connectionString);
                await connection.OpenAsync(cancellationToken);

                progress?.Report(new DeploymentProgress { Status = "Connection successful", Percentage = 100 });
                result.Success = true;
            ...
            catch (Exception ex)
            {
                ...
                progress?.Report(new DeploymentProgress { Status = result.Message, Percentage = 0, IsError = true });
            }
        ...
        public async Task<DeploymentResult> DeployDatabaseAsync(string sqlScriptPath, IProgress<DeploymentProgress> progress = null, CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                progress?.Report(new DeploymentProgress { Status = "Starting deployment...", Percentage = 0 });

                // Validate SQL script file exists
                if (!File.Exists(sqlScriptPath))
                {
                    result.Success = false;
                    result.Message = $"SQL script file not found: {sqlScriptPath}";
                    progress?.Report(new DeploymentProgress { Status = result.Message, Percentage = 0, IsError = true });
                    return result;
                }

                result.Log.AppendLine($"Using SQL script file: {sqlScriptPath}");

                // Drop existing database if present (handles partial/failed prior deployments)
                await DropDatabaseIfExistsAsync(_databaseName, progress, _cancellationTokenSource.Token);

                // Deploy using SQL scripts
                await DeploySqlScript(sqlScriptPath, progress, result, _cancellationTokenSource.Token);

                if (result.Success)
                {
                    // Run post-deploy SQL commands
                    await RunPostDeploySqlCommands(progress, result, _cancellationTokenSource.Token);

                    result.Message = "Deployment completed successfully";
                    progress?.Report(new DeploymentProgress { Status = result.Message, Percentage = 100 });
                }
            }
            ...
            catch (Exception ex)
            {
                ...
                result.Log.AppendLine($"Error: {ex.StackTrace}");
                progress?.Report(new DeploymentProgress { Status = result.Message, Percentage = 0, IsError = true });
            }
            finally
            {
        ...
        private async Task DropDatabaseIfExistsAsync(string databaseName, IProgress<DeploymentProgress> progress, CancellationToken cancellationToken)
        {
        ...
        private async Task DeploySqlScript(string sqlScriptPath, IProgress<DeploymentProgress> progress, DeploymentResult result, CancellationToken cancellationToken)
        {
            progress?.Report(new DeploymentProgress { Status = "Loading SQL script...", Percentage = 10 });

            var sqlContent = await File.ReadAllTextAsync(sqlScriptPath, cancellationToken);
            result.Log.AppendLine($"Loaded SQL script: {Path.GetFileName(sqlScriptPath)}");

            progress?.Report(new DeploymentProgress { Status = "Executing SQL script...", Percentage = 30 });
            ...
            foreach (var batch in batches)
            {
                ...
                currentBatch++;
                var batchProgress = 30 + (int)((currentBatch / (double)totalBatches) * 30);
                progress?.Report(new DeploymentProgress { Status = $"Executing batch {currentBatch}/{totalBatches}...", Percentage = batchProgress });
            ...
            result.Log.AppendLine("SQL script executed successfully");
            result.Success = true;
            progress?.Report(new DeploymentProgress { Status = "SQL script executed successfully", Percentage = 60 });
        }

        private async Task RunPostDeploySqlCommands(IProgress<DeploymentProgress> progress, DeploymentResult result, CancellationToken cancellationToken)
        {
            progress?.Report(new DeploymentProgress { Status = "Running post-deploy SQL commands...", Percentage = 70 });

            using var connection = new SqlConnection(_connectionString);
            await connection.OpenAsync(cancellationToken);

            // Embedded post-deploy SQL commands
            var postDeployCommands = GetPostDeploySqlCommands();

            foreach (var command in postDeployCommands)
            {
                ...
                {
                    await cmd.ExecuteNonQueryAsync(cancellationToken);
                    result.Log.AppendLine("✓ Executed post-deploy command successfully");
                }
                catch (Exception ex)
                {
                    result.Log.AppendLine($"⚠️ Post-deploy command warning: {ex.Message}");
                }
            }

            result.Log.AppendLine("✓ Post-deploy commands completed");
            progress?.Report(new DeploymentProgress { Status = "Post-deploy commands completed", Percentage = 90 });
        }

        private List<string> GetPostDeploySqlCommands()
        {
            // Embedded SQL commands to run after dacpac deployment
            return new List<string>
            {
                // Add initial data or configurations here
                 @"
BEGIN TRANSACTION
