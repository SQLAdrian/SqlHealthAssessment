...
namespace SqlHealthAssessment.Data.Services
{
    public class SqlWatchDeploymentService
    {
        ...
        public async Task<DeploymentResult> DeployDatabaseAsync(string dacpacPath, IProgress<DeploymentProgress> progress = null, CancellationToken cancellationToken = default)
        {
            var result = new DeploymentResult { Success = false };
            ...
            try
            {
                ...
                {
                    result.Success = false;
                    result.Message = $"Dacpac file not found: {dacpacPath}";
                    progress?.Report(new DeploymentProgress { Status = result.Message, Percentage = 0, IsError = true });
                    return result;
                }

                result.Log.AppendLine($"Using dacpac file: {dacpacPath}");

                // Deploy using DacFx
                await DeployBacpac(dacpacPath, progress, result, _cancellationTokenSource.Token);
        ...
        private async Task DeployBacpac(string dacpacPath, IProgress<DeploymentProgress> progress, DeploymentResult result, CancellationToken cancellationToken)
        {
            progress?.Report(new DeploymentProgress { Status = "Loading dacpac package...", Percentage = 10 });

            // Load the dacpac package
            var dacPackage = await Task.Run(() => DacPackage.Load(dacpacPath), cancellationToken);
            result.Log.AppendLine($"Loaded dacpac package: {dacPackage.Name}");
            ...
            var dacOptions = new DacDeployOptions
            {
                ...
                ExcludeObjectTypes = new[] { ObjectType.Users, ObjectType.Logins },
                DoNotDropObjectTypes = new[] { ObjectType.Users, ObjectType.Logins },
                IncludeTransactionalScripts = true
            };

            progress?.Report(new DeploymentProgress { Status = "Deploying database...", Percentage = 30 });
            ...
            result.Log.AppendLine("Database deployed successfully");
            result.Success = true;
            progress?.Report(new DeploymentProgress { Status = "Database deployed successfully", Percentage = 60 });
        }
