...
namespace SqlHealthAssessment.Data.Services
{
    ...
    public class PMInstallationService : IDisposable
    {
        ...
        /// <summary>
        /// Looks for install SQL files in PerformanceMonitor_db/install/ under the app base directory.
        ...
        public static (string? SqlDirectory, string? MonitorRootDir, List<string> SqlFiles) FindInstallationFiles()
        {
            ...
            // Primary: <appBase>/PerformanceMonitor_db/install/
            string primaryInstall = Path.Combine(appBase, "PerformanceMonitor_db", "install");
            if (Directory.Exists(primaryInstall))
            {
                var files = GetInstallFiles(primaryInstall);
                if (files.Count > 0)
                    return (primaryInstall, Path.Combine(appBase, "PerformanceMonitor_db"), files);
            }

            // Fallback: <appBase>/PerformanceMonitor_db/ (flat structure)
            string primaryFlat = Path.Combine(appBase, "PerformanceMonitor_db");
...
END;";
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            {
                cancellationToken.ThrowIfCancellationRequested();
                var (s, f) = await ExecuteUpgradeAsync(upgrade.Path, connectionString, progress, cancellationToken)
                    .ConfigureAwait(false);
                totalSuccess += s;
                totalFailed  += f;
            }
        ...
        private static async Task<(int, int)> ExecuteUpgradeAsync(
            ...
            CancellationToken cancellationToken)
        {
            ...
            progress?.Report(new PMInstallationProgress { Message = $"Applying upgrade: {upgradeName}", Status = "Info" });

            var fileNames = (await File.ReadAllLinesAsync(upgradeFile, cancellationToken).ConfigureAwait(false))
            ...
            foreach (string fileName in fileNames)
            {
                ...
                {
                    progress?.Report(new PMInstallationProgress { Message = $"  {fileName} - NOT FOUND", Status = "Warning" });
                    failed++;
                    continue;
                }
                ...
                {
                    progress?.Report(new PMInstallationProgress { Message = $"  {fileName} - FAILED: {ex.Message}", Status = "Error" });
                    failed++;
                }
            }
